...
def show_participants_list(message, bot, active_collections, test_collection, known_groups, user_sessions):
    if message.chat.type != "private":
        ...
        if storage:
            ...
            bot.reply_to(message, f"📊 **Статус сбора:**\n👥 Участников: {quantity}\n⏱️ Прошло: {int(passed // 60)} мин.", parse_mode="Markdown")
        else:
            bot.reply_to(message, "📭 Сейчас нет активного сбора. Историю смотрите в ЛС.")
        return
    ...
    if not groups:
        return bot.send_message(message.chat.id, "📭 **Групп не обнаружено**", parse_mode="Markdown")

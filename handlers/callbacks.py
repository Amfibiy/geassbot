...
def register_callbacks(bot, active_collections, test_collection, known_groups, user_sessions):
    ...
    def handle_my_chat_member_handler(update):
        handle_my_chat_member(update, bot, active_collections, test_collection, known_groups, user_sessions)
    
    @bot.message_handler(func=lambda message: message.chat.type in ['group', 'supergroup'] and not (message.text and message.text.startswith('/')))

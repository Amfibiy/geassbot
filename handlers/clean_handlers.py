...
from database.mongo import get_group_by_id, delete_history_record_by_id,get_combined_settings
from utils.validators import validate_date
from utils.helpers import check_cancellation, get_cancel_kbd,get_localized_timestamps
...
def register_clean_handlers(bot, active_collections, test_collection, known_groups, user_sessions):
    ...
    def handle_text_group_id(message):
        ...
        if not group:
            bot.reply_to(message, "❌ Группа с таким ID не найдена. Попробуйте еще раз или нажмите Отмена.", reply_markup=get_cancel_kbd())
            return
            
        name_group = group.get('title', f"Группа {group_id}")
    ...
    def handle_clean_choice(call):
        ...
        if choice in ['today', 'yesterday', 'week', 'month']:
            ...
            if choice == 'today':
                ...
                show_clean_weeks_menu(call, bot, b, e, "Месяц", back_cb="clean_back_to_periods")
            ...
            bot.send_message(call.message.chat.id, "⌨️ Режим ввода даты", reply_markup=get_cancel_kbd())
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="clean_back_to_periods"))
            bot.edit_message_text("✍️ <b>Введите период</b> (ДД.ММ.ГГ - ДД.ММ.ГГ):", 
    ...
    def handle_clean_specific_period(call):
        ...
        chat_id = session.get('clean_chat_id')
        session['clean_view_begin'] = float(data[2])
        session['clean_view_end'] = float(data[3])
        session['clean_view_label'] = data[4]
    ...
    def handle_clean_drilldown(call):
        ...
        if v_type == 'mview':
            session['clean_parent_mview'] = call.data
            show_clean_weeks_menu(call, bot, b_ts, e_ts, label, back_cb="clean_view_all")
        elif v_type == 'wview':
            session['clean_parent_wview'] = call.data
            back = session.get('clean_parent_mview', 'clean_back_to_periods')
            show_clean_days_menu(call, bot, b_ts, e_ts, label, back_cb=back)
        elif v_type == 'dview':
            back = session.get('clean_parent_wview', 'clean_back_to_periods')
            show_clean_hours_menu(call, bot, b_ts, e_ts, label, back_cb=back)
        
        bot.answer_callback_query(call.id)

    @bot.callback_query_handler(func=lambda call: call.data.startswith('clean_single_'))
    def handle_single_delete(call):
        ...
        if action == 'ask':
            rec_id = call.data.replace('clean_single_ask_', '')
            show_records_for_cleaning(call, bot, chat_id, begin, end, label, user_sessions, back_cb=back_cb, confirm_id=rec_id)
        elif action == 'yes':
            rec_id = call.data.replace('clean_single_yes_', '')
            if delete_history_record_by_id(rec_id):
                bot.answer_callback_query(call.id, "✅ Удалено")
            else:
                bot.answer_callback_query(call.id, "❌ Ошибка", show_alert=True)
            show_records_for_cleaning(call, bot, chat_id, begin, end, label, user_sessions, back_cb=back_cb)
        elif action == 'no':
            show_records_for_cleaning(call, bot, chat_id, begin, end, label, user_sessions, back_cb=back_cb)
    ...
    def handle_bulk_confirm(call):
        ...
        session = user_sessions.get(call.from_user.id, {})
        back_cb = session.get('clean_last_menu_cb', 'clean_back_to_periods')
        ask_confirm_clean(call, session.get('clean_chat_id'), int(float(parts[2])), int(float(parts[3])), "выбранный период", session, bot, back_cb=back_cb)

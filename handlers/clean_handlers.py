...
def register_clean_handlers(bot, active_collections, test_collection, known_groups, user_sessions):
    ...
    def handle_clean_specific_period(call):
        ...
        chat_id = session.get('clean_chat_id')
        show_records_for_cleaning(call, bot, chat_id, float(data[2]), float(data[3]), data[4], user_sessions)
        bot.answer_callback_query(call.id)
        ...
        if len(parts) >= 6:
            try:
                ...
                if d1 and d2:
                    ...
                    show_records_for_cleaning(message, bot, chat_id, begin, end, label, user_sessions)
                    session['step'] = "clean_choice_period"
                    return
            except Exception:
                pass
    ...
    def handle_single_delete(call):
        ...
        if delete_history_record_by_id(rec_id):
            ...
            show_records_for_cleaning(call, bot, session.get('clean_chat_id'), session.get('clean_view_begin'), session.get('clean_view_end'), session.get('clean_view_label'), user_sessions)
        else:
    ...
    def handle_bulk_confirm(call):
        ...
        session = user_sessions.get(call.from_user.id, {})
        ask_confirm_clean(call, session.get('clean_chat_id'), int(parts[2]), int(parts[3]), "выбранный период", session, bot)
